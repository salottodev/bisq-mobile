...
class BisqFirebaseMessagingServiceTest {
    ...
    fun `from classifies a chat message carrying only a channel id as a private message`() {
        ...
        assertEquals(
            BisqFirebaseMessagingService.PushNotification.PrivateChatMessage("notif-1", CHANNEL_ID, PEER),
            notification,
        )
    }
    ...
    fun `from treats a blank peer name as absent`() {
        ...
        assertEquals(
            BisqFirebaseMessagingService.PushNotification.PrivateChatMessage("notif-1", CHANNEL_ID, null),
            notification,
        )
    }
    ...
    @Test
    fun `chat messages redact to the category summary on the lock screen`() {
        listOf(
            ...
            // The same stand-in the local path uses, so the two cannot show different redacted copy.
            assertEquals(
                NotificationRedactions.chatMessage(),
                notification.lockScreen,
                "${notification::class.simpleName} names the peer, so it must redact",
            )
        }
    }

    @Test
    fun `notifications that name nobody are shown in full on the lock screen`() {
        // Including a chat message the node could not route: its banner is already the category
        // summary, so redacting it would hide information without hiding anything private.
        listOf(
            ...
            assertEquals(
                ...
                notification.lockScreen,
                "${notification::class.simpleName} carries no peer name, so it must not redact",
            )
        }
    }
    ...
    @Test
    fun `a chat push is built with the summary as its public form`() {
        val service = Robolectric.buildService(BisqFirebaseMessagingService::class.java).get()
        ...
        listOf(
            ...
            assertEquals("New message", public.extras.getString(Notification.EXTRA_TEXT), "must not name the peer")
            assertEquals(Notification.VISIBILITY_PUBLIC, public.visibility, "the stand-in is what gets shown")
        }
    }

    @Test
    fun `a push that names nobody is built without a public form`() {
        val service = Robolectric.buildService(BisqFirebaseMessagingService::class.java).get()
