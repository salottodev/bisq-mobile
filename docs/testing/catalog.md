...
- **Screens that apply `SecureScreenEffect`**: stay on `BisqComposeUiTestBase`. Render `CaptureHostWindow { window = it }` (`shared/presentation/.../security/HostWindow.kt`) next to the screen and assert `window.isSecure`. Exemplar: `SellerState1UiTest`.
