...
| `CaptureHostWindow`, `Window.isSecure` | `shared/test-utils/src/androidMain/kotlin/.../test/presentation/compose/HostWindow.kt` |
...
- **`SecureScreenEffectUiTest`** (`shared/presentation/.../security/SecureScreenEffectUiTest.kt`) must keep `createAndroidComposeRule<ComponentActivity>()` because it reads `composeTestRule.activity.window` / `FLAG_SECURE`. Do **not** force `BisqComposeUiTestBase` (bases expose only `createComposeRule()`). It needs no Koin: plain Robolectric with the default Application. Theme via `setBisqTestContent` only when needed.
- **Screens that apply `SecureScreenEffect`**: keep the leaf base the screen already needs. Render `CaptureHostWindow { window = it }` (`shared/test-utils/.../test/presentation/compose/HostWindow.kt`) next to the screen and assert `window.isSecure`. Exemplar: `SellerState1UiTest`.
...
- **Node Compose screens that need a mocked `koinInject` binding**: keep `BisqComposeUiTestBase` + `@Config(TestApplication)` and add the binding with `loadKoinModules` inside the test — the node has no `ClientInjectComposeUiTestBase`, and a base that owns `startKoin` would hit the double-`startKoin` above (the node `TestApplication` leaves its Koin running for the next class). Bind `NavigationManager` yourself when the screen renders `TopBar`: the node test module's relaxed mock cannot type `currentTab`. Exemplar: `BackupScreenUiTest`.
